...
function convertCompletionEntry(tsEntry, uri, position, context) {
  ...
  if (tsEntry.replacementSpan) {
    ...
    if (range.start.line != range.end.line) {
      range = import_coc10.Range.create(range.start, import_coc10.Position.create(range.start.line, context.line.length));
    }
    textEdit = {
      insert: range,
      replace: range,
      newText: (_a = tsEntry.insertText) != null ? _a : label
    };
  } else {
    if (context.isMemberCompletion && context.line.endsWith("#")) {
      let len = context.line.length;
      let range = import_coc10.Range.create(position.line, len - 1, position.line, len);
      textEdit = import_coc10.TextEdit.replace(range, (_b = tsEntry.insertText) != null ? _b : label);
    }
  }
